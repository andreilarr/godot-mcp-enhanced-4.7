...
export async function killOrphanGodotProcesses(projectDir: string): Promise<number> {
  ...
  const normalizedDir = projectDir.replace(/\\/g, '/').toLowerCase();
  ...
  if (process.platform === 'win32') {
    ...
    return new Promise((resolve) => {
      const ps = spawn('powershell', [
        ...
        `Where-Object { $_.CommandLine -like '*--path*' -and $_.CommandLine -like '*${normalizedDir}*' } | ` +
    ...
    // Linux/macOS: pgrep + cmdline 匹配
    return new Promise((resolve) => {
      const ps = spawn('pgrep', ['-f', `godot.*--path.*${normalizedDir}`], { stdio: ['pipe', 'pipe', 'pipe'] });
      let out = '';
      ps.stdout.on('data', (d: Buffer) => { out += d.toString(); });
      ps.on('close', () => {
        const pids = out.trim().split('\n').map(Number).filter(n => n > 0);
...
case 'stop_project': {
  if (!ctx.runningProcess) {
    // 第二层：扫描残留 Godot 进程
    const orphanKilled = await killOrphanGodotProcesses(ctx.projectDir || '');
    if (orphanKilled > 0) {
      return textResult(`Cleaned up ${orphanKilled} orphaned Godot process(es). Project directory: ${ctx.projectDir}`);
    }
